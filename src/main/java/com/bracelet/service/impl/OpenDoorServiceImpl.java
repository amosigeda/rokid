...
public class OpenDoorServiceImpl implements IOpenDoorService {
	...
	public List<OpenDoorInfo> getHistory(Long user_id, String imei,
		...
		List<OpenDoorInfo> list = jdbcTemplate.query(sql, new Object[] {
				user_id, imei, starttime, endtime },
	...
	public Integer getOpenCount(String imei) {
		int i = jdbcTemplate.queryForInt(
				"select count(id) AS id from open_door_info where imei = ? and TO_DAYS(createtime) = TO_DAYS(NOW())",
	...
	@Override
	@DataSourceChange(slave = true)
	public List<OpenDoorInfo> getAllHistory(String imei, String starttime,
			String endtime) {
		String sql = "select * from open_door_info where  imei =? and createtime>= ? and createtime<= ?  order by createtime desc";
		List<OpenDoorInfo> list = jdbcTemplate.query(sql, new Object[] { imei,
				starttime, endtime }, new BeanPropertyRowMapper<OpenDoorInfo>(
				OpenDoorInfo.class));
		return list;
	}
