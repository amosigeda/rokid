...
public class UserController extends BaseController {
	...
	@ResponseBody
	@RequestMapping(value = "/ask/device/{token}/{imei}", method = RequestMethod.GET)
	public HttpBaseDto askDevice(@PathVariable String token,@PathVariable String imei) {
		Long user_id = checkTokenAndUser(token);
		UserInfo userInfo = userInfoService.getUserInfoById(user_id);
		if (userInfo == null) {
	...
	public HttpBaseDto saveUserInfo(@RequestParam String token,
			@RequestParam String nickname, @RequestParam String head) {
		logger.info("修改个人信息token=" + token + "昵称=" + nickname + "头像=" + head);
		if (StringUtils.isAnyEmpty(nickname, nickname, head)) {
			throw new BizException(RespCode.NOTEXIST_PARAM);
		}
		Long user_id = checkTokenAndUser(token);
