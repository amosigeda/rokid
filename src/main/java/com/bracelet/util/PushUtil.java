...
public class PushUtil {
	...
	public static PushResponse pushNotify(String targetValue, String title, String content, String notifyContent,
			int second) {        logger.info("开始推送[通知][targetValue:" + targetValue + ",title:" + title + ",content:" + content + ",second:" + second + "]");
	        PushResponse result = null;
	        try {
	            DefaultAcsClient client = new DefaultAcsClient(profile);
	            PushRequest pushRequest = new PushRequest();
	    ...
	    //        pushRequest.setIOSExtParameters(content);
	            ...
	            pushRequest.setStoreOffline(true);
	            result = client.getAcsResponse(pushRequest);
	            logger.info("收到推送[通知]结果[targetValue:" + targetValue + ",title:" + title + ",content:" + content + ",second:" + second + "] -> RequestId: " + result.getRequestId() + ", MessageId:" + result.getMessageId());
	        } catch (Exception e) {
	            logger.info("推送发送错误:", e);
	        }
	        return result;
